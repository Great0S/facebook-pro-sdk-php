...
function sign_in_with_facebook()
{
    if (!is_user_logged_in()) {
        if (!get_option('users_can_register')) {
            return 'Registrations are closed for now!';
        } else {
            global $handler;
            $nonce = wp_create_nonce("facebook_sign_in_nonce");
            $link = admin_url('admin-ajax.php?action=facebook_sign_in&nonce=' . $nonce);
            $redirect_to = $link;
            $data = ["email"];
            $fullUrl = $handler->getLoginUrl($redirect_to, $data);
            return "<button onclick=\"window.location.href='$fullUrl';\">Facebook Login</button>";
        }
    } else {
...
function facebook_sign_in()
{
    ...
    if (!email_exists($userEmail)) {
        ...
        $newUserID = wp_insert_user(array(
            ...
            'billing_address_1' => isset($userData['location']) ? $userData['location'] : '',
            'profile_photo' => isset($userData['picture']['url']) ? $userData['picture']['url'] : '',
            'role' => 'subscriber'
        ));
        if ($newUserID) {
            ...
            // New user logging in
            do_action('wp_login', $userSignIn, $userEmail);
